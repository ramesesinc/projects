...
public class SpecialCollectionAdapter extends BaseAdapter 
{
	...
	public View getView(int position, View view, ViewGroup parent) {
		...
		//System.out.println("special collection id: " + item.get("objid").toString());
		...
		ImageView icon = (ImageView) v.findViewById(R.id.iv_specialcollection_approved);
		icon.setImageResource(R.drawable.btn_check_off);
//		System.out.println("state -> "+item.get("state").toString());
		if (item.get("state").toString().equals("DOWNLOADED")) {
			v.setClickable(false);
			icon.setImageResource(R.drawable.btn_check_on);
		}
		return v;
	}
