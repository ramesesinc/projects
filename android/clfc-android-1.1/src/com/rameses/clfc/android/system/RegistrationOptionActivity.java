...
public class RegistrationOptionActivity extends SettingsMenuActivity 
{
	...
	private void doNext() {
		...
		if (tag.equals("register")) {
			...
			int networkStatus = ((ApplicationImpl) Platform.getApplication()).getNetworkStatus();
			if (networkStatus == 3) {
				String terminalid = TerminalManager.getTerminalId();
				if (terminalid == null || terminalid.length() == 0) {
					if (progressDialog.isShowing()) progressDialog.dismiss();
					ApplicationUtil.showShortMsg("Terminal not registered");
					return;
				}
...
//				if (myhandler == null) myhandler = new Handler();
				...
				new Thread(new RecoveryProcess()).start();
				
//				new Thread(new Runnable() {
//					public void run() {
//						myhandler.postAtTime(new RecoveryProcessTask(), 300);
//					}
//				}).start();
				
//				Platform.getTaskManager().schedule(new Runnable() {
//					public void run() {
//						myhandler.postAtTime(new RecoveryProcess(), 300);
//					}
//				});
//				Platform.runAsync(new RecoveryProcess());
	...
	private void println(String msg) {
		Log.i("RegistrationOptionActivity", msg);
	}
