...
public class SettingsActivity extends UIActionBarActivity 
{
	...
	private void save() {
		...
		if (isEmpty(onlinehost)) {
			requestFocus(R.id.et_host_online);  
			ApplicationUtil.showShortMsg("Host for mobile is required", SettingsActivity.this);
			
		} else if (isEmpty(offlinehost)) {
			requestFocus(R.id.et_host_offline); 
			ApplicationUtil.showShortMsg("Host for WIFI is required", SettingsActivity.this);
			
		} else if (isEmpty(port)) {
			requestFocus(R.id.et_host_port); 
			ApplicationUtil.showShortMsg("Port is required", SettingsActivity.this);
			
		} else if (isEmpty(sessiontimeout)) {
			requestFocus(R.id.et_timeout_session); 
			ApplicationUtil.showShortMsg("Session timeout is required", SettingsActivity.this);
			
		} else if (isEmpty(uploadtimeout)) {
			requestFocus(R.id.et_timeout_upload); 
			ApplicationUtil.showShortMsg("Upload timeout is required", SettingsActivity.this);
			
		} else if (isEmpty(trackertimeout)) {
			requestFocus(R.id.et_timeout_tracker); 
			ApplicationUtil.showShortMsg("Tracker timeout is required", SettingsActivity.this);
			...
			getHandler().post(new Runnable() {
				public void run() {
					if (progressDialog.isShowing()) progressDialog.dismiss();
					ApplicationUtil.showShortMsg("Settings successfully saved!", SettingsActivity.this);
				}
			});		
		}
	}
