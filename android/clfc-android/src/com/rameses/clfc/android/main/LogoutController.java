...
//	private UIActivity activity;
	...
	private boolean hasUnpostedTransactionsImpl(DBContext paymentdb, DBContext remarksdb, DBContext clfcdb) throws Exception {
	...
	private class LogoutActionProcess implements Runnable 
	{
		...
		private void processCollection(SQLTransaction clfcdb, String itemid) throws Exception {
			...
			while (!sheets.isEmpty()) {
				...
				objid = MapProxy.getString(data, "objid");
				clfcdb.delete("notes", "parentid=?", new Object[]{objid});
				clfcdb.delete("collectionsheet", "objid=?", new Object[]{objid});
			} 
		}
	}	
}
