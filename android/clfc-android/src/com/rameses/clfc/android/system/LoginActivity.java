...
import com.rameses.clfc.android.services.LoanTrackerService;
		...
		public void run() { 
			...
			try {
				...
				if ("logout".equals(collector_state)) {
					...
					map.put("captureid", "CP" + UUID.randomUUID().toString());
					map.put("collector_state", "login");
					map.put("trackerid", "TRCK" + UUID.randomUUID().toString());
