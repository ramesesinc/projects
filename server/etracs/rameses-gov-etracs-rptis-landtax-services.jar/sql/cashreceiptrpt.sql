...
update rptledgeritem_qtrly rliq, rptledger_payment rp,  rptledger_payment_item rpi set
...
FROM (
	...
	FROM cashreceipt cr
		...
		INNER JOIN rptledger_payment_item rpi ON cr.objid = rpi.rptreceiptid
		INNER JOIN rptledger rl ON rp.rptledgerid = rl.objid 
		INNER JOIN sys_org b ON rl.barangayid = b.objid
		inner join sys_org md on md.objid = b.parent_objid 
