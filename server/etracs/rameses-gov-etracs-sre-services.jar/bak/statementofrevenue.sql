...
[getSreSubAccounts]
SELECT objid, parentid, code, title, type , 
	(SELECT target FROM sreaccount_incometarget WHERE objid=s.objid AND year=$P{year} limit 1) AS target
FROM sreaccount s
WHERE parentid = $P{parentid} 
  AND type IN ('group', 'detail')
ORDER BY code 
...
SELECT t.*,
	(SELECT target FROM account_incometarget WHERE objid=t.objid AND year=$P{year} limit 1) AS target
FROM (
	SELECT 
...
SELECT t.*,
	(SELECT target FROM account_incometarget WHERE objid=t.accountid AND year=$P{year} limit 1) AS target
FROM (
	SELECT 
		CASE 
...
SELECT t.*,
	(SELECT target FROM sreaccount_incometarget WHERE objid=t.objid AND year=$P{year} limit 1) AS target
FROM (
	SELECT 
	CASE WHEN acct.objid IS NULL THEN 'unmapped' ELSE acct.objid END AS objid,
	CASE WHEN acct.parentid IS NULL THEN 'unmapped' ELSE acct.parentid END AS parentid,
...
SELECT t.*,
	(SELECT target FROM sreaccount_incometarget WHERE objid=t.accountid AND year=$P{year} limit 1) AS target
FROM (
	SELECT 
		CASE 
			WHEN subacct.parentid IS NOT NULL THEN subacct.parentid 
