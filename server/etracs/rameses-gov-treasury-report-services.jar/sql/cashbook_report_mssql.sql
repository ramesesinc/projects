...
from ( 
	select 
		...
		('AF '+ formno +'#'+ min(refno) +'-'+ max(refno)) as refno, 
	...
	select 
		...
		min(('*** VOIDED - AF '+ c.formno +'#'+ c.refno +' ***')) as refno, sum(c.dr) as dr, sum(c.cr) as cr, 
	...
	select 
		...
		('REMITTANCE - '+ r.liquidatingofficer_name) as particulars, 
	...
	group by 
		r.controldate, r.controlno, r.dtposted, ('REMITTANCE - '+ r.liquidatingofficer_name) 
