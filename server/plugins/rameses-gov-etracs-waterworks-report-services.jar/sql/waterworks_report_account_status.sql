...
from ( 
	select 
		...
		case 
			when wa.classificationid='RESIDENTIAL' and wa.state = 'ACTIVE' and ifnull(wm.state,'') <> 'DISCONNECTED' then 1 else 0 
		end as res_active, 
		case 
			when wa.classificationid='COMMERCIAL' and wa.state = 'ACTIVE' and ifnull(wm.state,'') <> 'DISCONNECTED' then 1 else 0 
		end as com_active, 
		case 
			when wa.classificationid='INDUSTRIAL' and wa.state = 'ACTIVE' and ifnull(wm.state,'') <> 'DISCONNECTED' then 1 else 0 
		end as ind_active, 
		case 
			when wa.classificationid='BULK' and wa.state = 'ACTIVE' and ifnull(wm.state,'') <> 'DISCONNECTED' then 1 else 0 
		end as bulk_active, 
		case 
			when wa.classificationid='GOVERNMENT' and wa.state = 'ACTIVE' and ifnull(wm.state,'') <> 'DISCONNECTED' then 1 else 0 
		end as gov_active, 
		case 
			when wa.classificationid='RESIDENTIAL' and ifnull(wm.state,'') = 'DISCONNECTED' then 1 
			when wa.classificationid='RESIDENTIAL' and wa.state = 'INACTIVE' then 1 
			else 0 
		end as res_inactive,  
		case 
			when wa.classificationid='COMMERCIAL' and ifnull(wm.state,'') = 'DISCONNECTED' then 1 
			when wa.classificationid='COMMERCIAL' and wa.state = 'INACTIVE' then 1 
			else 0 
		end as com_inactive,  
		case 
			when wa.classificationid='INDUSTRIAL' and ifnull(wm.state,'') = 'DISCONNECTED' then 1 
			when wa.classificationid='INDUSTRIAL' and wa.state = 'INACTIVE' then 1 
			else 0 
		end as ind_inactive,  
		case 
			when wa.classificationid='BULK' and ifnull(wm.state,'') = 'DISCONNECTED' then 1 
			when wa.classificationid='BULK' and wa.state = 'INACTIVE' then 1 
			else 0 
		end as bulk_inactive, 
		case 
			when wa.classificationid='GOVERNMENT' and ifnull(wm.state,'') = 'DISCONNECTED' then 1 
