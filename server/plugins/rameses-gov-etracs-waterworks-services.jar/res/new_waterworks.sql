...
CREATE TABLE `waterworks_account` (
  ...
  `lastdateread` datetime DEFAULT NULL,
  ...
  KEY `ix_metersizeid` (`metersizeid`),
  ...
  CONSTRAINT `waterworks_account_ibfk_5` FOREIGN KEY (`metersizeid`) REFERENCES `waterworks_metersize` (`objid`),
  CONSTRAINT `waterworks_account_ibfk_6` FOREIGN KEY (`sectorid`) REFERENCES `waterworks_sector` (`objid`),
  CONSTRAINT `waterworks_account_ibfk_7` FOREIGN KEY (`stuboutid`) REFERENCES `waterworks_stubout` (`objid`),
...
/*Table structure for table `waterworks_billing_account` */

DROP TABLE IF EXISTS `waterworks_billing_account`;

CREATE TABLE `waterworks_billing_account` (
  `objid` varchar(50) NOT NULL,
  `parentid` varchar(50) DEFAULT NULL,
  `state` varchar(50) DEFAULT NULL,
  `acctid` varchar(50) DEFAULT NULL,
  `prevyear` int(11) DEFAULT NULL,
  `prevmonth` int(11) DEFAULT NULL,
  ...
  PRIMARY KEY (`objid`),
  UNIQUE KEY `uix_billing_account_parent_acctid` (`parentid`,`acctid`),
  CONSTRAINT `fk_billing_account_parent` FOREIGN KEY (`parentid`) REFERENCES `waterworks_billing` (`objid`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
CREATE TABLE `waterworks_stubout_node` (
  ...
  KEY `ix_applicationid` (`applicationid`),
  KEY `ix_acctid` (`acctid`),
  CONSTRAINT `waterworks_stubout_node_ibfk_1` FOREIGN KEY (`acctid`) REFERENCES `waterworks_account` (`objid`),
