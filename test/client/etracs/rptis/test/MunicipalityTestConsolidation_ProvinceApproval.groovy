class MunicipalityTestConsolidation_ProvinceApproval
{
    public static void runTest(faas1){
        ...
        * MUNICIPALITY: CREATE CONSOLIDATION 
        ...
        /*=================================================
        * PROVINCE: APPROVED SUBMITTED CONSOLIDATION
        =================================================*/
        println '='*50
        println '[province] APPROVING submitted consolidation from municipality'
        def provsvc = ProvinceTestProxy.create('RPTISProvinceTestMunicipalityCSForApproval')
        TestHelper.waitForConsolidation(consolidation, provhelper)
        def municonsolidation = munisvc.getConsolidationForTesting(consolidation)
        println provsvc.testSubmittedConsolidationForApproval(consolidation, municonsolidation)

        consolidation += provsvc.openConsolidation(consolidation)
        task = provsvc.doReceive(consolidation)
        ...
        provsvc.approveConsolidation(task)
        println 'approveConsolidation...done'
        println provsvc.testApprovedConsolidation(consolidation)
        ...
        * MUNICIPALITY: TEST APPROVED SUBDIVISION
        =================================================*/
        println '='*50
        println '[municipality] Test APPROVED consolidation from province'
        TestHelper.waitForApprovedConsolidation(consolidation, munihelper)
        println munisvc.testApprovedConsolidation(consolidation)
